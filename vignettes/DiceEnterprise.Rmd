...
```
Also the method `sample` allows to set the option `verbose = TRUE`. In this case the output of `sample` is a list where the first argument is the obtained sample and the second argument is a vector containing the number of rolls required. The same method allows to specify the number of cores used (default 1). Notice that multicore is not supported on Windows. The method `sample` takes also `double_time` as an input. If `double_time = FALSE` (default), the time step of CFTP is increased by one at each iteration leading, otherwise it is doubled.
```{r bf_toss_2}
...
ce2 <- CoinsEnterprise$new(f_indep_coins2, toss.coins = toss.until.heads, die_type = "first_heads")
```
...
result <- ce2$sample(n = 1000, num_cores = 2, verbose = TRUE, double_time = TRUE, probs = indep_coins_probs) #the argument probs is passed to toss.coins
print(table(result[[1]])/1000) #Empirical probabilities. Notice that the theoretical ones are given by
print(indep_coins_probs/sum(indep_coins_probs))
print(paste0("Average number of rolls required: ", mean(result[[2]])))
