...
$$
```{r bf_settings}
sample_size <- 1 #Number of tosses
...
bf.res.eff <- function(toss.coin, original_ladder,max_increase_degree) {
  ...
  for(i in 1:length(probs_vec)) {
    ...
    df_res[counter,] <- c(probs_vec[i],0,bf_res[1,i],bound_res[1,i])
    counter <- counter + 1
    for(deg in 1:max_increase_degree) {
      ...
      df_res[counter,] <- c(probs_vec[i],deg,bf_res[deg+1,i],bound_res[deg+1,i])
      counter <- counter + 1
    }
  }
  return(list(bf_res,bound_res,df_res))
}
...
```{r bf_efficiency_plot, echo=FALSE}
df_res <- aux[[3]]
ggplot(data = df_res, aes(x = p, y = bound_tosses, colour = factor(degree))) +
  geom_line(aes(group = factor(degree))) + geom_point()
```
